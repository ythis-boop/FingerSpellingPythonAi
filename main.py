...
model_letter_path = f"test1"
model_number_path = f"test2"
onnx_path = f"aslmodel_randomfc.onnx"
# onnx_path = f"classifier/model_quantized.onnx"
...
def recognize_gesture(image, results):
    ...
    if results.multi_hand_landmarks:
        ...
        for idx in reversed(range(len(multi_hand_landmarks))):
            ...
            prediction = sess.run([label_name], {input_name: [np.asarray(data_aux)]})[0]
...
if __name__ == '__main__':
    ...
    with mp_hands.Hands(
        ...
        while capture.isOpened():
            ...
            try:
                image = recognize_gesture(image, results)
